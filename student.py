...
def update_treeview():
    ...
    # Make sure to consume any unread result before executing the next query
    cursor.fetchall()  # Fetch any previous unread results (even if not needed)

    # Retrieve data from the database
    query = "SELECT * FROM attendance WHERE student_name = %s"
    cursor.execute(query, (studentName,))  # Execute the query to get data for this student
    rows = cursor.fetchall()  # Fetch the data for this query
